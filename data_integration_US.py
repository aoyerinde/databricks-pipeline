...
try: 
  url = 'https://pkgstore.datahub.io/core/bond-yields-us-10y/monthly_json/data/216cf74b1d210338816ade152dd627b4/monthly_json.json'
  api = DatabhubAPI(url)
...
final_df.registerTempTable("us_bond_yield_df") 
...
# MAGIC %sql 
# MAGIC CREATE TABLE IF NOT EXISTS RAW_DATABASE.US_MONTHLY_BOND_YIELDS  (REPORT_DATE DATE, RATE FLOAT, SLICE_TIME TIMESTAMP, INSERT_TIME TIMESTAMP);

# COMMAND ----------

sqlContext.sql("DELETE FROM raw_database.us_monthly_bond_yields WHERE report_date >= '{}' and report_date <= '{}'".format(slices[0],slices[1]))

# COMMAND ----------

sqlContext.sql("INSERT INTO raw_database.us_monthly_bond_yields (select * from us_bond_yield_df)")

# COMMAND ----------


# remove dbfs artifacts incase you have ran this multiple times
#dbutils.fs.rm("dbfs:/user/hive/warehouse/raw_database.db/us_monthly_bond_yields", True)